...
stellar_backend_json() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    python3 - "$timeout_seconds" "$script" "$stellar_action" "$ROOT" "$@" <<'PY'
import os
import signal
import subprocess
import sys
...
collect_email_lists_json() {
  if [ -z "${STELLAR_DESKTOP_BACKEND-}" ] && command -v python3 >/dev/null 2>&1; then
    python3 - "$ROOT" <<'PY'
import glob
import json
import os
...
root = sys.argv[1]
mail_lists = ["accepted", "quarantine", "spam", "banned", "archive", "sent", "outbox", "trash"]
all_lists = mail_lists + ["spam-review"]
...
def scalar(path, key, default=""):
    return unquote(path.get(key, default))

def bool_value(value):
    return str(value).strip().lower() in ("true", "yes", "1", "on")

def attachment_count(raw):
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as handle:
            for line in handle:
                ...
                if not line.startswith((" ", "\t")):
                    ...
                    key = key.strip()
                    value = value.strip()
                    if key in ("headers_cache", "render"):
                        section = key
                    else:
                ...
                if section and ":" in stripped:
                    key, value = stripped.split(":", 1)
                    target = headers if section == "headers_cache" else render
                    target[key.strip()] = unquote(value)
    except OSError:
        return {}
    data["headers_cache"] = headers
    data["render"] = render
    return data

def sidecars_for_list(list_name):
    list_dir = os.path.join(root, list_name)
    if not os.path.isdir(list_dir):
        return
    for pattern in (".*.yml", "*.yml"):
        for sidecar in glob.glob(os.path.join(list_dir, pattern)):
            if os.path.isfile(sidecar):
                yield "", sidecar
    for sender_dir in glob.glob(os.path.join(list_dir, "*")):
        if not os.path.isdir(sender_dir) or os.path.basename(sender_dir) == "attachments":
            continue
        sender = os.path.basename(sender_dir)
        for pattern in (".*.yml", "*.yml"):
            for sidecar in glob.glob(os.path.join(sender_dir, pattern)):
                if os.path.isfile(sidecar):
                    yield sender, sidecar

def message_from_sidecar(list_name, sender, sidecar):
    data = parse_sidecar(sidecar)
    headers = data.get("headers_cache", {})
    render = data.get("render", {})
    from_value = scalar(headers, "from", sender)
    sender_value = sender or from_value
    return {
        "list": list_name,
        "sender": sender_value,
        "ulid": scalar(data, "ulid"),
        "subject": scalar(headers, "subject", scalar(data, "subject")),
        "from": from_value,
        "to": scalar(headers, "to"),
        "received_at": scalar(data, "received_at", scalar(headers, "date")),
        "status": scalar(data, "status_shadow", list_name),
        "read": bool_value(data.get("read", "false")),
        "starred": bool_value(data.get("starred", "false")),
        "pinned": bool_value(data.get("pinned", "false")),
        "attachments": attachment_count(data.get("attachments", "")),
        "rspamd_score": scalar(data, "rspamd_score"),
        "llm_spam_probability": int(data["llm_spam_probability"]) if str(data.get("llm_spam_probability", "")).isdigit() else None,
        "llm_spam_category": scalar(data, "llm_spam_category"),
        "llm_spam_reason": scalar(data, "llm_spam_reason"),
        "llm_spam_source": scalar(data, "llm_spam_source"),
        "llm_spam_model": scalar(data, "llm_spam_model"),
        "preview": "",
        "eml_path": "",
        "html_path": scalar(render, "html"),
        "plain_path": scalar(render, "plain"),
    }

messages_by_list = {name: [] for name in all_lists}
for name in mail_lists:
    for sender, sidecar in sidecars_for_list(name) or ():
        messages_by_list[name].append(message_from_sidecar(name, sender, sidecar))
messages_by_list["spam-review"] = messages_by_list["spam"] + messages_by_list["banned"]

result = []
for name in all_lists:
    result.append({
        "id": name,
        "messages": sorted(messages_by_list[name], key=lambda item: item.get("received_at", ""), reverse=True),
    })
