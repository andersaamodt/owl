...
fn parse_email(body: &[u8]) -> Result<ParsedEmail> {
    ...
    let mut result = ParsedEmail {
        ...
        to: header_values(&parsed, &["To", "X-Stellar-Envelope-To"]),
...
fn header_values(parsed: &ParsedMail<'_>, names: &[&str]) -> Vec<String> {
    parsed
        .headers
        .iter()
        .filter(|header| {
            names
                .iter()
                .any(|name| header.get_key_ref().eq_ignore_ascii_case(name))
        })
...
mod tests {
    ...
    fn deliver_to_route_sets_status_and_path() {
        with_fake_sanitizer("#!/bin/sh\nsed 's/<script/[blocked]/g'\n", || {
            ...
            assert_eq!(
                sidecar.headers_cache.to,
                vec![
                    "Personal <me@example.org>".to_string(),
                    "me@example.org".to_string()
                ]
            );
            assert_eq!(
                sidecar.headers_cache.cc,
                vec!["archive@example.net".to_string()]
            );
            assert_eq!(
                sidecar.headers_cache.date,
                "Tue, 25 Jul 2026 12:00:00 +0000"
            );
